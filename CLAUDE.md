...
**В группе и канале бот отвечает только на `/anons_here`** (`GROUP_COMMANDS`
в `src/index.ts`). Это не косметика: у бота-администратора Telegram отключает
режим приватности, поэтому он получает все сообщения чата — реакция на них
(«не знаю такой команды», шаги диалогов) превращалась бы в спам. Карточки,
заявки и диалоги — только в личке.
