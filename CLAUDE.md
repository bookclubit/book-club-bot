...
### Регистрации (диплинки из miniapp, состояние в D1)

- `/start join_<eventId>` — запись на встречу: бот сразу присылает ссылки
  (созвон, доска, материалы, трансляции) и напоминает в день встречи (cron).
- `/start speaker_<eventId>` — заявка на доклад: клавиатура тем главы эфира
  (🔒 — занята; уникальность брони гарантирует индекс D1) или «своя тема»
  текстом. Заявка уходит админу (`ADMIN_CHAT_ID`) с кнопками
  подтвердить/отклонить; решение приходит спикеру. Контент (доклад в событии,
  спикер в реестре) после подтверждения оформляется через CMS, как обычно.
- `GET /api/claims?event=<eventId>` — публичный JSON занятости тем (CORS `*`),
  для miniapp.

Принцип: **контент — в git (book-club-data), оперативное состояние
(брони, записи) — в D1.** Обработчики — `src/handlers/registration.ts`,
...
- `ADMIN_CHAT_ID` — chat_id админа для модерации заявок спикеров
  (`wrangler secret put ADMIN_CHAT_ID`). Без него заявки копятся в D1
  без уведомлений.
